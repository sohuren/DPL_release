...
# Test settings with the pytorch
...
parser.add_argument('--dataroot', type=str, default='../release/data/drug_gene/0',
                    help='the data root')
parser.add_argument('--test_data', type=str, default='raw_sentence',
                    help='test data')
...
parser.add_argument('--initial_model', type=str,  default='./model/model.pkl10',
                    help='the current entity type we are trained on')
parser.add_argument('--out_file', type=str,  default='./result/mlp_vis.txt',
...
def GetResult(data, entity_type, vocab):
    ...
    for line in data:
        ...
        drug_mentions = line[1].split() # mention index for drug
        drug_mentions = [int(x) for x in drug_mentions]
        gene_mentions = line[2].split() # mention index for gene
        gene_mentions = [int(x) for x in gene_mentions]
...
# ignore this first
Result = GetResult(data, args.entity_type, vocab)

print (Result)
# visualization the result using the visualizer
print ("writing the result to text \n")
make_text_file(Result, args.out_file)
