...
def make_html_file(examples, filename, class_entity):
    ...
    def write_context(out_file, context, matches, class_entity, example_index):
        ...
        for idx, word in enumerate(context):
            ...
            # match[2] is the predicted label
			if current_class:
			...
			if current_class and not other_class:
				...
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' :class_entity+":"+ type_name, 'rgb' : attention_to_rgb(0.5), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
				out_file.write('</u>')
			elif current_class and other_class:
				out_file.write('<u>')
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' :class_entity+"+"+category+":"+type_name, 'rgb' : attention_to_rgb(1), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
				out_file.write('</u>')
			else:
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
...
def make_html_file_confidence(examples, filename, gene_key):
    ...
    def write_context(out_file, context, matches, example_index):
        ...
        for idx, word in enumerate(context):
			...
			if current_class and pred==0:
				out_file.write('<u>')
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' : "gene:" + id, 'rgb' : attention_to_rgb(1), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
				out_file.write('</u>')
			elif current_class and pred==1:
    				out_file.write('<u>')
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' : "disease", 'rgb' : attention_to_rgb(1), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
				out_file.write('</u>')
			elif current_class and pred==2:
    				out_file.write('<u>')
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' : "None", 'rgb' : attention_to_rgb(1), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
				out_file.write('</u>')
			elif current_class and pred==3:
    				out_file.write('<u>')
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' : "No context", 'rgb' : attention_to_rgb(1), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
				out_file.write('</u>')
			else:
				out_file.write('<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                           {'pointer_text' : "no matching", 'rgb' : attention_to_rgb(0), 'word' : word, 'class' : str(example_index) + "-" + word.replace("@", "")})
    ...
    out_file.write("<p><H2> %s </H2></p>"%("Disease Example"))    
    for instance in examples['positive']:
        ...
        out_file.write("<p><b> 0: %f 1: %f 2: %f </b></p>"%(item[3][0][0], item[3][0][1], item[3][0][2]))
        write_context(out_file, context, item, example_idx)
        out_file.write("</p>")
        example_idx += 1

    out_file.write("<p><H2> %s </H2></p>"%("Gene Example"))    
    example_idx = 0
    ...
    out_file.write("<p><H2> %s </H2></p>"%("None Example"))    
    example_idx = 0

    for instance in examples['none']:
