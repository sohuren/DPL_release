...
def bern_bern_kl(y_pred,y_true):
    eps = 1e-20
...
'''
we need to modify this one to coporate the kl divergence objective
'''
class LogitRegression(Chip):
    ...
    def compute(self, input_tv, sample_weight=None):
        
        if input_tv.ndim == 3:
            self.output_tv = T.nnet.softmax(T.dot(input_tv.max(axis=0), self.W) + self.b)
            self.gold_y = T.fmatrix(make_name(self.name, 'gold_y')).astype('float32') # batch * output Layer
            if sample_weight:
                print ("inside Logit")
                print (sample_weight)
                self.score = T.mean(T.nnet.categorical_crossentropy(self.output_tv, self.gold_y)*sample_weight)
            else:
                self.score = T.mean(T.nnet.categorical_crossentropy(self.output_tv, self.gold_y))
            ...
            self.gold_y = T.fvector(make_name(self.name, 'gold_y')).astype('float32') # batch * output Layer
            if sample_weight:
                print ("inside Logit")
                print (sample_weight)
                self.score = T.mean(T.nnet.categorical_crossentropy(self.output_tv, self.gold_y)*sample_weight)
