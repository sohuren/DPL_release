...
def gen_graph_from_json(infile, outfile, graphfile, labelfile):
    import json

    if os.path.exists(labelfile):
        fp = open(labelfile, "r")
        relations = pickle.load(fp)
        fp.close()
    total_label = len(relations)
        ...
        for line in inf:
            ...
            for item in content:
                ...
                if relation == "None":
                    ...
                    label = [0, 1]
                relation_sen = " ".join([str(i) for i in label])
                outf.write(re.sub('\t', ' ', ' '.join(origin_text))+'\t'+'\t'.join([' '.join(map(str,idx)) for idx in indices])+'\t'+ relation_sen +'\n')
                grf.write(' '.join(dep_arcs)+'\n')
...
def prepare_data(seqs, eidxs, mask=None, weights=None, maxlen=None):
    ...
    if weights is not None:
        x_weights = numpy.zeros((n_samples)).astype(theano.config.floatX)
    ...
    if weights is not None:
        return x, x_mask, np_eidxs, x_weights
    else:
...
def load_data_cv(data_dir, folds, dev_fold, test_fold=None, arc_type_dict=dict(), num_entities=2, dep=False, content_fname='sentences', dep_fname='graph_arcs', add=True, additional=False, sample_iter=0, factor_set=1):
    corpus_size = {}
    corpus = []

    additional_sample = False
    additional_sample_graph = False

    for i in range(folds):
        if i == dev_fold:
            fname = "sentences_2nd" # always read data sentences_2nd
            print (fname)
            #fname = "annotation3" # now change to mannual annotation
            #print (fname)
            sub_corpus = read_file(os.path.join(data_dir, str(i), fname), num_entities)
            all_instances = range(len(sub_corpus[0])) # all the instances
        if test_fold is not None and i == test_fold:
            ...
            corpus_x, corpus_y, corpus_idx = sub_corpus

            # only load one time from test_fold
            if os.path.exists(os.path.join(data_dir, str(test_fold), "sentences_2nd_additional" + str(sample_iter) + str(factor_set) )) and additional_sample == False and additional:
                sub_corpus_additional = read_file(os.path.join(data_dir, str(test_fold), "sentences_2nd_additional" + str(sample_iter) + str(factor_set) ), num_entities)
                corpus_x_additional, corpus_y_additional, corpus_idx_additional = sub_corpus_additional
                corpus_x += corpus_x_additional
                corpus_y += corpus_y_additional
                corpus_idx += corpus_idx_additional
                additional_sample = True

            sub_corpus = (corpus_x, corpus_y, corpus_idx)

        if dep:
            dependencies, arc_type_dict = read_graph_dependencies(os.path.join(data_dir, str(i), dep_fname), arc_type_dict, add)    
            
            # only load one time from test_fold
            if os.path.exists(os.path.join(data_dir, str(test_fold), "graph_sentences_2nd_additional"+str(sample_iter) + str(factor_set) )) and additional_sample_graph == False and additional:
                dependencies_additional, arc_type_dict = read_graph_dependencies(os.path.join(data_dir, str(test_fold), "graph_sentences_2nd_additional"+str(sample_iter) + str(factor_set) ), arc_type_dict, add)
                dependencies += dependencies_additional
                additional_sample_graph = True
    ...
    if test_fold is not None:
        return [train, valid, test, dics, corpus_size]
    return [train, valid, dics, corpus_size]
...
#        urlStr = 'http[s]?://(?:[a-zA-Z]|[1-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
    with cs.open(filename, 'r', encoding='utf-8') as inf:
		...
		sys.stderr.write('id:' + str(line_count)+'\n')
            ...
            y = [float(i) for i in label.split()]
            if y[0] < y[1]:
                y = numpy.asarray([0, 1], dtype=numpy.float32)
            else:
                y = numpy.asarray([1, 0], dtype=numpy.float32)
...
if __name__ == '__main__':
    ...
    gen_graph_from_json(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
