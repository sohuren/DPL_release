...
def create_relation_circuit(_args, StackConfig):
    ...
    if _args.graph or _args.batch_size > 1:
        ...
        print ("test")
        print (cargs)
        x_w, mask, idx_arr, sample_weights, y, y_pred, cost, grads, regularizable_params = _args.circuit(cargs)
        inputs = idx_arr + [x_w, mask, sample_weights, y]
        pred_inputs = [x_w, mask] + idx_arr
    else:
        x_w, idx_arr, sample_weights, y, y_pred, cost, grads, regularizable_params = _args.circuit(cargs)
        inputs = idx_arr + [x_w, sample_weights, y]
